...
def paper_tokenize(text, sentences_as_lists=False, preserve_order=False):
    # Implementation derived from: https://github.com/EdCo95/scientific-paper-summarisation
    """
...
def compute_tf(word: str, dict_freq: Counter, total_words):
    word_freq = dict_freq[word]
    
    return word_freq / total_words

def compute_isf(word: str, no_sentences: int, freq_dict: Counter):
    if word in freq_dict:
        sentence_freq = freq_dict[word]
        return math.log10(no_sentences / sentence_freq)
    return 0

def compute_tfisf(tf: float, isf: float):
    return tf * isf
...
def compute_sentence_weight(sentence_pos, sentence, dict_freq, no_sentences, no_words):    
    sentence_score = 0
    ...
    for word, pos in sentence_pos:
        ...
        for syn_word in synonyms:
            tf = compute_tf(syn_word, sentence, no_words)
            isf = compute_isf(syn_word, no_sentences, dict_freq)
            sentence_score += compute_tfisf(tf, isf)
...
def summarize_paper(tokenized_paper, tokenized_paper_pos, paper_sentences, nr_sentences=5):
    ...
    for tok_sentence_pos, tok_sentence, orig_sentence in zip(processed_sentences_pos, processed_sentences, original_sentences):
        ...
        sentence_weight = compute_sentence_weight(tok_sentence_pos, tok_sentence, freq_dict, no_sentences, no_words)
...
if __name__ == '__main__':
    ...
    for i, paper_file_name in tqdm(enumerate(paper_file_names)):
        ...
        with open(f'{SUMMARY_PATH}/tfisf_wordnet/{paper_file_name}', 'w') as sum_file:
